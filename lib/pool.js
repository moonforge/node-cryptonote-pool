...
setInterval(function(){
    ...
    for (var minerId in connectedMiners){
        ...
        if (now - miner.lastBeat > timeout){
            log('warn', logSystem, 'Miner timed out and disconnected %s@%s', [miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip]);
...
function Miner(id, login, pass, ip, startingDiff, pushMessage){
    this.id = id;
    var address = login.split('.');
    this.login = address[0];
    this.worker = parseInt(address[1], 10) | 0;
    this.pass = pass;
    this.ip = ip;
...
Miner.prototype = {
    ...
    setNewDiff: function(newDiff){
        ...
        log('info', logSystem, 'Retargetting difficulty %d to %d for %s', [this.difficulty, newDiff, this.login + (this.worker ? '.' + this.worker : '')]);
    ...
    checkBan: function(validShare){
        ...
        if (stats.validShares + stats.invalidShares >= config.poolServer.banning.checkThreshold){
            if (stats.invalidShares / stats.validShares >= config.poolServer.banning.invalidPercent / 100){
                log('warn', logSystem, 'Banned %s@%s', [this.login + (this.worker ? '.' + this.worker : ''), this.ip]);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    var redisCommands = [
        ...
        ['hincrby', config.coin + ':sharesWorker:roundCurrent', miner.login + (miner.worker ? '.' + miner.worker : ''), job.score],
        ['zadd', config.coin + ':hashrate', dateNowSeconds, [job.difficulty, miner.login, dateNow].join(':')],
        ['zadd', config.coin + ':hashrateWorker', dateNowSeconds, [job.difficulty, miner.login + (miner.worker ? '.' + miner.worker : ''), dateNow].join(':')],
        ['hincrby', config.coin + ':workers:' + miner.login, 'hashes', job.difficulty],
        ['hincrby', config.coin + ':workersWorker:' + miner.login + (miner.worker ? '.' + miner.worker : ''), 'hashes', job.difficulty],
        ['hset', config.coin + ':workers:' + miner.login, 'lastShare', dateNowSeconds],
        ['hset', config.coin + ':workersWorker:' + miner.login + (miner.worker ? '.' + miner.worker : ''), 'lastShare', dateNowSeconds]
    ];
    ...
    log('info', logSystem, 'Accepted %s share at difficulty %d/%d from %s@%s', [shareType, job.difficulty, shareDiff, miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip]);
...
function processShare(miner, job, blockTemplate, nonce, resultHash){
    ...
    if (hash.toString('hex') !== resultHash) {
        log('warn', logSystem, 'Bad hash from miner %s@%s', [miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip]);
        return false;
    }
    ...
    if (hashDiff.ge(blockTemplate.difficulty)){
        ...
        apiInterfaces.rpcDaemon('submitblock', [shareBuffer.toString('hex')], function(error, result){
            if (error){
                log('error', logSystem, 'Error submitting block at height %d from %s@%s, share type: "%s" - %j', [job.height, miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip, shareType, error]);
                ...
                log('info', logSystem,
                    ...
                    [blockFastHash.substr(0, 6), job.height, miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip, result]
                );
        ...
        log('warn', logSystem, 'Rejected low difficulty share of %s from %s@%s', [hashDiff.toString(), miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip]);
        return false;
    }
    else{
        recordShareData(miner, job, hashDiff.toString(), false, null, shareType);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            var address = params.login.split('.');
            var worker = parseInt(address[1]) | 0;
            log('info', logSystem, 'Miner connected %s@%s',  [address[0] + (worker ? '.' + worker : ''), miner.ip]);
            break;
        case 'getjob':
            if (!miner){
        ...
        case 'submit':
            ...
            if (!noncePattern.test(params.nonce)) {
                var minerText = miner ? (' ' + miner.login + (miner.worker ? '.' + miner.worker : '') + '@' + miner.ip) : '';
            ...
            if (shareTrustEnabled){
                if (shareAccepted){
                    ...
                    log('warn', logSystem, 'Share trust broken by %s@%s', [miner.login + (miner.worker ? '.' + miner.worker : ''), miner.ip]);
