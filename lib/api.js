...
var redisCommands = [
    ...
    ['zremrangebyscore', config.coin + ':hashrateWorker', '-inf', ''],
    ['zrange', config.coin + ':hashrateWorker', 0, -1],
];
    ...
    async.parallel({
        pool: function(callback){
            redisClient.multi(redisCommands).exec(function(error, replies){
                ...
                var hashratesWorker = replies[12];
                ...
                for (var i = 0; i < hashratesWorker.length; i++){
                    var hashPartsWorker = hashratesWorker[i].split(':');
                    minerStatsWorker[hashPartsWorker[1]] = (minerStatsWorker[hashPartsWorker[1]] || 0) + parseInt(hashPartsWorker[0]);
                }
                ...
                for (var minerWorker in minerStatsWorker){
                    var sharesWorker = minerStatsWorker[minerWorker];
                    minerStatsWorker[minerWorker] = getReadableHashRateString(sharesWorker / config.api.hashrateWindow);
                }
...
function broadcastLiveStats(){
    ...
    for (var address in addressConnections){
        var miner = address.split('.');
        var worker = parseInt(miner[1]) | 0;
        if(worker) {
            redisCommands.push(['hgetall', config.coin + ':workersWorker:' + miner[0] + (worker ? '.' + worker : '')]);
        } else {
    ...
    redisClient.multi(redisCommands).exec(function(error, replies){
        ...
        for (var i = 0; i < addresses.length; i++){
            ...
            if(worker) {
                stats.hashrate = minerStatsWorker[miner[0] + (worker ? '.' + worker : '')];
            } else {
...
function handleWorkerMinerStats(urlParts, response){
    ...
    if (urlParts.query.longpoll === 'true'){
        redisClient.exists(config.coin + ':workersWorker:' + miner[0] + (worker ? '.' + worker : ''), function(error, result){
            if (!result){
        ...
        redisClient.multi([
            ['hgetall', config.coin + ':workersWorker:' + miner[0] + (worker ? '.' + worker : '')],
            ['zrevrange', config.coin + ':payments:' + miner[0], 0, config.api.payments - 1, 'WITHSCORES']
...
function handleMinerStats(urlParts, response){
    ...
    var miner = urlParts.query.address.split('.');
    if(miner.length === 2) {
        handleWorkerMinerStats(urlParts, response);
        return;
    }
